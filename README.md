...
### 매출 성장 속 AOV 정체 원인 탐색과 개선 레버 도출
...
거래건수는 증가하는 상황에서 AOV를 통해 추가 성장 레버로 활용할 수 있는지를     
...
- **데이터 선택 이유** : 해당 데이터는 거래 데이터뿐 아니라 고객 속성, 상품 카테고리, 프로모션 정보, 앱/웹 행동 로그를 함께 포함하고 있어  
...
- 거래건수와 매출은 지속적으로 증가했지만 AOV는 일정 범위에 머무르는 패턴.
- AOV가 상승하지 않는 원인 후보를 검토하고, **추가 성장 레버로 활용할 수 있는 개선 지점을 탐색**.
...
- **홈페이지 UX 개선** : ITEM_DETAIL 미진입률 개선하기 위해 상품 카드 CTA, 퀵뷰, 추천 영역 개선 제안
...
- **재구매 고객 번들 추천 고도화**: 5회 이상 재구매 고객 대상 맞춤 번들 추천으로 AOV와 LTV 개선 가능성 검토
...
- AOV 월별 급등락 구간을 별도로 확인했으나, 시뮬레이션 데이터 특성상 특정 변동을 고객군·상품군·프로모션 요인으로 명확히 설명하기 어려움.
